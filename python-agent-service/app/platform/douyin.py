...
class DouyinAdapter(PlatformAdapter):
    ...
    def _paid_real(self, platform_order_id: str) -> PaymentCheck:
        ...
        # TODO: data = self._request("order.orderDetail", {"order_id": platform_order_id})
        #       detail = data["data"]
        #       paid = bool(detail.get("pay_amount")) or detail.get("pay_status") in (...)
        #       return PaymentCheck(paid, "平台确认：买家已付款" if paid
        #           else "平台复核：订单仍未付款")
        self._request("order.orderDetail", {"order_id": platform_order_id})
        raise ConfigError("抖音付款复核结果解析尚未实现（见 app/platform/douyin.py TODO）")

    def _ship_order_real(
        self, platform_order_id: str, logistics_company: str, waybill_no: str
    ) -> ShipResult:
        self.require_ready()
        # TODO: 调抖店发货 API（如 order.logisticsAdd），回传平台发货状态。
        self._request(
            "order.logisticsAdd",
            {"order_id": platform_order_id, "company": logistics_company, "logistics_code": waybill_no},
        )
        raise ConfigError("抖音真实发货回写尚未实现（见 app/platform/douyin.py TODO）")
    ...
    def _publish_listing_real(self, payload: PublishListingPayload) -> PublishResult:
        self.require_ready()
        # TODO: 将 payload.product_plan / image_plan / video_url 映射为抖店商品字段，
        #       调用抖店商品创建/编辑 API，成功后返回平台商品 ID 和商品链接。
        self._request(
            "product.add",
            {
                "outer_product_id": payload.product_id,
                "name": payload.product_plan.get("recommended_title") or payload.product_name,
            },
        )
        raise ConfigError("抖音真实发布尚未实现：请在 app/platform/douyin.py 的 _request 与 _publish_listing_real 中接入商品发布 API")
