...
### 1.4 平台策略（已与用户确认）
- **首打平台：小红书**——优先把"想法 → 上架"全链路在小红书跑通。
- **淘宝 / 抖音：留位处理**——多选界面中灰显标"即将上线"，结构/导出保留，暂不实现生成与发布。
- 文生图、小红书真发布**是核心路线（必做）**，不再视为"可延后"。
...
**目录优先流程（2026-07-12 确认）**：用户勾选「商品」tab 已建好的商品 + 选中目标平台（仅小红书可勾，淘宝/抖音灰显）→ 后端按商品 id 从 Java 拉真实数据喂 Agent，不再手填想法（修掉原 `NewProductIdea` 别名漏字段触发 `products` NOT NULL 500）。
1. **选商品+平台**（Step 0，多选）。
2. **商品文案生成**（ProductPlanningAgent）：标题、卖点、详情、SEO 词、按选中平台生成对应文案（含平台文案归一化）；可注入分类知识库（RAG）。
3. **图片生成**（ImageCreativeAgent）：主图/场景图/营销图提示词 + 风格；附**本地视觉合规审核**。真出图待接图生图 API（M1）。
4. **人审 → 发布 / 导出**：
   - **发布**：审核通过后一键发布到选中的平台（先只小红书，M3 接开放平台 API）。
...
- **LLM 打底 + 规则降级**：5 个 Agent 默认走本地 Ollama 结构化输出，失败降级到规则实现并继续。
...
**通用组件（自写，不引第三方 UI 库）**：`JsonView`（递归渲染 JSON）、`step-bar`（步骤进度条）、`platform-multiselect`（小红书可选/其他灰显）、`product-multiselect`（勾选商品，可按品类筛选）、`confirm-actions`（通过/驳回）、`LineChart`（手写 SVG 折线图）、`client.ts`（`api.get/post`）、`types.ts`（统一 `Json`）。各页统一 `loading`/`empty`/`error` 占位。
...
| `/new-listing` | 新品上架（线1，目录优先） | `step-bar`：①选商品+平台 ②文案审批 ③图片审批 ④完成上架；逐个商品处理；通过/驳回门控；发布按钮 M3 前灰显，仅留导出 |
...
| 阶段 | 名称 | 目标 | 关键决策 | 状态 |
|---|---|---|---|---|
| 0 | LLM 打底 | 5 Agent 接真实 LLM + 规则降级 | `LLM_*` 指向本地 Ollama；结构化输出复用 Schema | ✅ |
...
| **M1 图生图** | 接图生图 API，替换占位框，真出图（小红书 3:4） | 用户的图生图 API | ⏳ 待接入 |
| **M2 审核强化** | 图真生成后、发布前终审门控 | M1 | ⏳ |
| **M3 小红书发布** | 接小红书开放平台发布 API，真上架一篇笔记 | 小红书商家权限（AppKey+Secret+OAuth） | ⏳ |
| **M4 留位** | 淘宝/抖音多选灰显、导出结构保留、发布暂不实现 | M3 | ⏳ |
...
- ✅ 线1 真实状态：文案 Agent 成功（按选中平台生成+归一化）；图片仅提示词+合规审核✅、真生成⏳（M1）；人审门控✅；发布❌（M3）；M0 端到端收尾✅（已知 500 已修）。
...
## 十、后续路线图（方向）
1. **M1 文生图**：占位框 → 真实图片生成。
2. **M3 小红书真发布**：接开放平台，文案/图片一键发布（需专业号/商家凭证）。
3. **M4 其他平台扩位**：淘宝/抖音从"留位"转为可选+可发布（复用线一框架）。
