...
> 最近一次更新：2026-08-16。当前主链路、企业/店铺/成员体系和权限基础已完成，下一阶段是平台 API 真实联调与生产验收。
...
3. **真实结果优先、显式模式**：文案、图片、视频等生成能力缺 Key 或调用失败时直接提示，不返回模板或假媒体；规则模式与订单模拟必须由用户或部署显式选择。
4. **落库契约稳定**：扩展字段向后兼容，Java 侧通用 `Map`/`json` 承载 Agent JSON。
5. **轻薄前端**：React SPA 只调 Java REST（线1 走 `/agent` 经代理），不引重型 UI 库；登录、企业、店铺和成员权限由 Java 统一鉴权。

### 1.4 平台策略（已与用户确认）
- **三平台同步推进**：小红书 / 淘宝 / 抖音都是线一正式目标平台，不再做"小红书优先、淘宝/抖音灰显留位"。
- **三平台均为正式目标平台**：小红书、淘宝、抖音可分别生成、审核和发布，平台凭证按店铺隔离；具体外部发布仍等待官方 API 联调。
- **目标是 API 接入后可直接使用**：线一保留平台字段、平台文案、平台订单来源和发布闸门；当前三平台适配器仍需完成官方 OAuth、请求映射和真实账号联调。
...
|---|---|---|---|
| **Supervisor** | 流程总控 | 固定顺序编排 4 子 Agent、汇总、生成 `final_summary`、标 `manual_review_required`/`errors`；建 `trace_id` | 动态路由（trigger_type→步骤表）、LLM 失败重试 1 次后返回可读错误 |
...
**失败处理**：每个 Agent `try/except` 记入 `errors`；显式规则模式走确定性规则，真实生成模式失败则中断并返回可读错误，不伪造生成结果；`agent_runs` 落 `input_json`/`output_json`/`status`/`error`。
...
**早期阶段记录**：真实电商 API、复杂权限、多店铺多租户等曾属于后续范围；当前企业、店铺、成员、权限和内部履约闭环已完成，剩余重点是三平台官方 API 联调与生产部署。
...
| **M3 三平台发布适配器** | 统一发布任务、幂等、重试和对账骨架已完成；分别补小红书 / 淘宝 / 抖音官方请求映射 | 各平台商家权限（AppKey+Secret+OAuth） | 🟡（真实 OAuth、请求映射和账号联调待接） |
| **M4 多平台发布复用** | 同一商品可分别按三平台生成/确认/发布，复用线一单平台流程 | M3 | ⏳ |

---

## 九、当前完成状态（截至 2026-08-16）
...
- ✅ 线1 真实状态：每次任务处理一个商品和一个目标平台，但小红书 / 淘宝 / 抖音三个平台都能同时存在并分别发起任务；ContentBrief、图片、文案、视频、人审门控、运营计划、发布幂等任务与对账已接入；真实三方平台请求映射仍待 M3。
...
## 十、后续路线图（方向）
1. **三平台官方发布映射**：复用已完成的统一发布任务与幂等层，分别实现小红书 / 淘宝 / 抖音开放平台请求和响应映射。
...
- **不伪造生成结果**：真实 LLM / 图片 / 视频 / 平台调用失败必须记录错误并向用户提示；规则和模拟仅作为显式选择，真实模式不得静默回退。
